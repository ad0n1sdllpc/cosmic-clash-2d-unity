...
class BasketTree {
    ...
    public static DecisionTree CreateTree() {
		...
		DecisionTree GO_PREDICT_FALL = new DecisionTree("GO_PREDICT_FALL", (state, data) => {data["_y"] = -3.65; data["_m"] = state["ballVelY"] / (state["ballVelX"] != 0 ? state["ballVelX"] : 0.1); data["_b"] = state["ballPosY"] - data["_m"] * state["ballPosX"]; data["_x"] = (data["_y"] - data["_b"]) / data["_m"]; data["_x"] = data["_x"] > 8.5 ? 8.5 - (data["_x"] - 8.5) : data["_x"]; data["_x"] = data["_x"] + 0.3;}, null, new List<DecisionTree> {MOVE_TO_CUSTOM_POS}, null);

		DecisionTree JUMP = new DecisionTree("JUMP", (state, data) => data["jump"] = 1, null, null, null);

		// Is in range to jump?
		DecisionTree JUMP_TO_BALL = new DecisionTree("JUMP_TO_BALL", null, (state, data) => Math.Abs(state["selfPosX"] - state["ballPosX"]) < 1 &&  Math.Abs(state["selfPosY"] - state["ballPosY"]) < 3 && Math.Abs(state["selfPosY"] - state["ballPosY"]) > 0.4, new List<DecisionTree> {JUMP}, null);
