...
class FootballTree {
    ...
    public static DecisionTree CreateTree() {
		...
		// Ball on the ground/high up and player behind it?
		DecisionTree GET_OVER_BALL = new DecisionTree("GET_OVER_BALL", (state, data) => {data["_x"] = 6.7;  data["y_between_ball"] = Math.Abs(state["ballPosY"] - state["selfPosY"]);}, (state, data) => Math.Abs(state["ballPosX"] - state["selfPosX"]) < 1 && ( Math.Abs(state["ballVelY"]) < 4 ||  (  data["y_between_ball"] >= 1 && data["y_between_ball"] <= 1.5 ) ), new List<DecisionTree> {JUMP, MOVE_TO_CUSTOM_POS}, new List<DecisionTree> {MOVE_TO_CUSTOM_POS});
